"""
weak_signal_params_grid_search.py

Per-dataset XGBoost hyperparameter search using an expanded grid.
...
For each parameter combination the score is the mean wRRF NDCG@10 across
10 repeated random 80/20 splits of the train+dev portion (same Monte Carlo
CV approach used throughout the project).  The combination with the highest
mean CV score wins.  The final model is retrained on the full 85 % and
evaluated once on the 15 % test set.

Output:
  data/results/per_dataset_best_params.csv
...
def main():
    ...
    for ds_name in dataset_names:
        ...
        # Fixed 85/15 traindev/test split (reproducible per dataset)
    ...
    csv_path = os.path.join(results_folder, "per_dataset_best_params.csv")
